...
import { EvidencePanel } from "@/components/hold/evidence-panel";
...
import { integrateEvidenceVerdict } from "@/packages/evidence/policy";
...
export default function HoldClient() {
  ...
  async function handleVerifyEvidence() {
    ...
    const markEvidenceUnavailable = () => {
      setEvidence(null);
      if (result) {
        setResult({
          ...result,
          verdict: integrateEvidenceVerdict(result.verdict, "UNAVAILABLE"),
    ...
    try {
      ...
      const data = (await response.json()) as EvidenceResponse | EvaluationErrorResponse;
      if (!response.ok || "error" in data) {
        markEvidenceUnavailable();
        setEvidenceError("message" in data ? data.message : "Evidence could not be retrieved. Please try again.");
        return;
      }
      if (result) {
        setResult({
          ...result,
          verdict: integrateEvidenceVerdict(result.verdict, data.status),
      ...
      markEvidenceUnavailable();
      setEvidenceError("The evidence connection was interrupted. Please try again.");
    } finally {
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-6 sm:py-8 flex flex-col">
        ...
        <section className="hero text-center max-w-2xl mx-auto mb-6 sm:mb-8" id="judge">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-muted/60 border border-border/60 text-xs font-semibold text-muted-foreground mb-3">
            ...
            <span>Pre-send judgment with inspectable evidence</span>
          </div>
          ...
          <p className="mt-2.5 text-sm sm:text-base text-muted-foreground max-w-lg mx-auto font-normal leading-relaxed">
            HOLD decides whether a message is ready to leave the room and flags factual claims that need evidence.
          </p>
        </section>
            ...
            <div className="lg:col-span-7 order-1 lg:order-2">
              <div className="flex flex-col gap-4">
                <VerdictPanel
                  ...
                  onJudgeAnother={handleJudgeAnother}
                ...
                {evidenceLoading && (
                  <div
                    ...
                    aria-live="polite"
                  >
                    Selecting a claim and checking retrieved sources…
                  </div>
                )}
                {evidenceError && (
                ...
                {evidence && <EvidencePanel result={evidence} />}
              </div>
            </div>
