...
import type { EvaluationResponse, Verdict } from "@/packages/core";
...
export function VerdictPanel({ result, onEditDraft, onJudgeAnother }: VerdictPanelProps) {
    ...
    >
      ...
      <CardHeader className="gap-3 pb-3">
        <div className="flex items-center justify-between">
          ...
          <Badge
            ...
            className={`text-xs font-semibold px-2 py-0.5 ${config.bgClass} ${config.colorClass} border-transparent`}
          >
            {config.title}
          </Badge>
        </div>
      ...
      <CardContent className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-2 rounded-xl bg-muted/40 p-3 border border-border/50">
          <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Why this verdict</span>
          {result.reasonLabels.length > 0 ? (
      ...
      <CardFooter className="flex flex-wrap gap-2 pt-2 border-t border-border/50 bg-muted/20">
        ...
        <Button variant="outline" size="sm" disabled={isDownloading} onClick={handleDownload} className="h-8 text-xs">
